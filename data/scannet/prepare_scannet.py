...
def get_instance_ids(objectId2segs, seg2verts, sem_labels):
    objectId2labelId = {}
    instance_ids = np.ones(
        shape=(len(sem_labels))) * -1  # -1: points are not assigned to any objects ( objectId starts from 0)
...
def process_one_scan(scan, cfg):
    ...
    torch.save({'mesh': mesh, 'aligned_mesh': aligned_mesh, 'sem_labels': sem_labels, 'instance_ids': instance_ids,
               ...
               os.path.join(cfg.SCANNETV2_PATH.split_data, cfg.split, scan + '.pth'))


def process_all_scans(cfg):
    SCAN_NAMES = sorted([line.rstrip() for line in open(f'meta_data/scannetv2_{cfg.split}.txt')])

    process_map(partial(process_one_scan, cfg=cfg), SCAN_NAMES, chunksize=1)
