...
class PipelineDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        for j in range(self.chunk_size):
            for i, gt_id in enumerate(instance_bbox_ids):
                if bbox_label[i] == 1 and gt_id == object_id_list[j]:
                    ...
                    ref_box_corner_label = get_3d_box(instance_bboxes[i, 0:3], instance_bboxes[i, 3:6]).astype(
                        np.float32)
        ...
        data["heading_class_label"] = angle_classes.astype(
            np.int64)  # (num_instance,) with int values in 0,...,NUM_HEADING_BIN-1
        data["heading_residual_label"] = angle_residuals.astype(np.float32)  # (num_instance,)
        data["size_class_label"] = size_classes.astype(
            np.int64)  # (num_instance,) with int values in 0,...,NUM_SIZE_CLUSTER
        ...
        data["gt_bbox"] = get_3d_box_batch(instance_bboxes[:, 0:3], instance_bboxes[:, 3:6], angle_classes).astype(
            np.float32)  # (num_instance, 8, 3)
        ...
        # grounding GT
        data["ref_box_label"] = np.array(ref_box_label_list).astype(
...
def scannet_collate_fn(batch):
    ...
    for key in batch[0].keys():
        if key in ['locs', 'locs_scaled', 'feats', 'sem_labels', 'instance_ids', 'num_instance', 'instance_info',
                   'instance_num_point', 'gt_proposals_idx', 'gt_proposals_offset', "instances_bboxes_tmp"]:
            continue
